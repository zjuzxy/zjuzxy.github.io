...
var gLocation = window.location.href,
    ...
    // 微信支持在线客服 gyh20180425
var gCf = getQueryString("cf"),
	...
	.done(function(data, status){
		if(data.result == "0"){
			...
			if(data.ifShowButton=="1"){
				if(is_app()||(data.ifWx==1&&is_weixin())){  // 微信支持在线客服 gyh20180425
				  // $("#barrage").show();
				  $("body").append(kfHtml);
				  var state=0;
				  //classification information 由接口返回
				  var classification=data.classfication;
				  var information=data.information;
				  var robotAnswer=data.keyWord;
				  // var question=data.keyWord;
				  // var rucan="question="+question; //问题入参
				  var rucan="source=571080007"+"&classification="+encodeURIComponent(classification)+"&information="+encodeURIComponent(information)+"&robotAnswer="+encodeURIComponent(robotAnswer); //问题入参
				      ...
				      state=1;
				    	...
				    	if(is_app()){
					      ...
					      webLink("https://wap.zj.10086.cn/new/mobileStore/customerService/toCustomerService.do",rucan,"在线客服");//20170714 关于在线客服整体迁移至在线公司客服系统的需求 by bester
						  	...
						  	// window.location=data.wxUrl+"?cf="+gCf+"&session="+gSession+"&nonce="+gNonce+"&encpn="+gEncpn+"&num="+gNum+rucan
						  	window.location=data.wxUrl
						  }
				    }
				    setTimeout(function(){
